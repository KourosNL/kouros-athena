resource "local_file" "lambda_info_to_s3" {
  content = templatefile("${path.module}/templates/info.js.tftpl", {
    response_message = jsonencode({
      domain = local.product.domain
      name   = local.product.name
      output = {
        location = "${aws_s3_bucket.aws_s3_bucket.arn}/output/data/"
      }
    })
  })
...
resource "aws_lambda_function" "lambda_info" {
  function_name     = "${local.product.domain}_${local.product.name}_info"
...
resource "aws_apigatewayv2_api" "lambda_info" {
  name          = "${local.product.domain}_${local.product.name}_info"
  protocol_type = "HTTP"
}
